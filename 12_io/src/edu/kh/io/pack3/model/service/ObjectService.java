...
public class ObjectService {
	 ...
	 * 
	 * -Java 객체를 입/출력 할 때 사용하는 바이트 "보조" 스트림 (보조스트림은 단독 사용 불가!)
	 * 
	 * *** 직렬화 (Serializable)*** - 객체를 직렬 형태로 변환 -직렬화란 객체의 상태(필드..)를 바이트 스트림으로 변화하는
	 * 과정 이를 통해 객체는 메모리에서만 존재하는 것이 아니라, 파일로 저장되거나 네트워크르르 통해 전송될 수 있음
	 * 
	 * -역질렬화는 바이트 스트림을 다시 객체로 복원하는 과정 즉, 저장되는거나 전송된 데이터를 원래의 객체 형태로 되돌리는 작업
	 * 
	 */
	...
	public void objectOutput() {
		...
		try {
			...
			// -> 직렬화 되지 않음
			// -> Member 클래스를 직렬화하기 위해 implements Serialzable(멤버 클래스에서 만들어야함) 해줘야함!
