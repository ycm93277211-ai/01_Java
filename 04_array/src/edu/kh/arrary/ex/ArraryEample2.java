...
//		만드는 복사 방법
		...
		int[] arr = { 1, 2, 3, 4, 5 };// arr은 주소만 갖고 있음 / 즉 공유 집 같은 느낌
...
//		깊은복사  -> 주소는 완전 다름 걍 집 모양 집에 있는 가구만 같음

//		->같은 자료형의 새로운 배열을 만들어서
//		기존배열의 데이터를 모두 복사하는 방법
...
//		1. for문을 이용한 깊은 복사
		...
		System.out.println("copyArr 값: " + Arrays.toString(copyArr1));

//		2.System.arraycop(원본배열명,원본복사 시작 인덱스,복사배열명,복사배열의 삽입 시작 인덱스,복사길이
		int[] copyArr2 = new int[arr.length];
		System.arraycopy(arr, 0, copyArr2, 0, arr.length);
		System.out.println("copyArr2 값: " + Arrays.toString(copyArr2));

//		3.복사할 참조변수 = Arrays.copyOf(원본배열명, 복사할)
...
//		값 변경 후 확인
		...
		copyArr2[2] = 999;
		copyArr3[2] = 5000;
		System.out.println("arr 값: " + Arrays.toString(copyArr2));
...
//		Arrays.sort(배열명): 배열 내 요소값들이 오름차순 정렴됨
		Arrays.sort(lotto);
