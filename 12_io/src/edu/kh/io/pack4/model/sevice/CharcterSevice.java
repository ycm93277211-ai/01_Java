package edu.kh.io.pack4.model.sevice;
...
public class CharcterSevice {

	/*
	 * 문자 기반 스트림 - 2byte 문자 단위로 입/출력 하는 스트림
	 * 
	...
	/**
	 * 문자 기반 파일 입력(파일 읽어오기) + 보조 스트림 Buffered 사용
	 */
	public void fileInput() {
		...
		try {
			// 문자 기반 스트림 생성
			fr = new FileReader("/io_test/20251014/노래가사.txt");
			// 보조 스트림 생성
			...
			while (true) {
				// String BufferedReader.readLine() : 한 줄씩 문자열을 읽은 역할(줄 단위)
				line = br.readLine();

				if (line == null)
					break;// 더 이상 읽어올 내용이 없다면 반복 종료
	...
	/**
	 * 문자 기반 파일 출력(파일 쓰기) + 보조 스트림 Buffered 사용
	 */
