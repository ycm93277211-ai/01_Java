...
public class SetService {
	...
	/*
	 * Set(집합,주머니 느낌) -기본적으로 순서를 유지하지 않음(index 없음!)!!!!!!!!!!!!!!!!!!!!!
	 * 
	 * -중복 데이터 저장 X !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 * 
	 * Set 인터페이스의 자식 클래스 1.HashSet(대표): 처리 속도가 빠른 Set 2.LinkedHashSet: 순서를 유지하는 Set
	 * 3.TreeSet: 자동 정령되는 Set
	 * 
	 */

	public void method1() {
		// HasSet 객체 생성(new 연산자 통해 만들어짐)
		Set<String> set = new HashSet<>();
//		Set< String > set= new TreeSet<>(); 
//		Set< String > set= new LinkedHashSet<>(); 

		// 1. boolean add(E e): 요소(e,E) 추가
		set.add("네이버");
		set.add("카카오");
		set.add("라인");
		set.add("쿠팡");
		...
		// (자동정령을 위해 내부적으로 compareTo() 라는 메서드 이용시 에러 발생하기 때문)
		set.add(null);
		set.add(null);
		set.add(null);
		set.add(null);
		...
		// 전달받은 인자 e를 Set에서 제거
		// 제거 성공 true,제거 대상 없으면 false
		System.out.println(set.remove("배달의민족"));// 있어서 true
		System.out.println(set);
		System.out.println(set.remove("유플러스"));// 없어서 false

		// 4.d boolean contains(E e):
	 ...
	 * 
	 * 1. Iterator(반복자)이용 2. List 로 변환 3. 향상된 for문 이용
	 * 
	 */
	public void method2() {
	...
	/**
	 * 직접 만들 클래스(Person)를 이용해 만든 객체를
	 * Set에 저장 (중복 제거 확인)
	 */
	public void method3() {
		//***************************************************************
		// Hash 라는 단어가 포함된 컬렉션 이용 시
		// 해당 객체 클래스에(ex Person)hashCode(), equals() 오버라이딩 필수적으로 진행되어야 함!!!!
		//***************************************************************
	 ...
	 * ->기본 오름차순 정렬
	 * 
	 * *TreeSet안에 저장된 객체는 Comparable(~비교되는, 유사한) 인터페이스 상속 필수!
	 * ->컬렉션 저장될 수 있는 형태는 객체만!
	 * ->int ->Integer 라는 Wrapper 클래스 이용하여 저장해야함.
	 * TreeSet<Integer>
	 * ->Integer 안에 Comparable 인터페이스 상속 되어야함!(자바에서 자동으로 상속 되어있음)
	 * 즉 바로사용 가능! (자동정렬됨!)
	 * 
	 * 
	 */
