...
public class ServerService {
	...
	// 소켓은 이 연결의 끝점 역할을 하며 , 데이터 송수긴 통로가 됨
	// 즉, TCP통신 == 소켓 간 연결 + 데이터 순서

	/*
	 * TCP - 서버, 클라이언트간 1:1 소켓통신 (연결 지향적 프로토콜) - 서버가 먼저 실행되어 있는 상태에서 클라이언트가 연결해야 함.
	 * 
	 * 
	 ...
	 * 
	 * Socket - 프로세스의 양 끝단(프로세스간의 통신을 담당) - Input/OutputStream 보유하고 있음.
	 *
	 *
	 * ServerSocket - 포트와 연결되어 외부 요청을 기다리는 객체 - 클라이언트의 연결 요청이 들어오면 클라이언트의 소켓을 얻어와
	 * 연결하게 된다.
	 *
	 * - 하나의 포트에는 하나의 ServerSocket만 연결 가능 -> 1대1매치
	 * 
	 *
	 */
	...
	public void serverStart() {
		...
		try {
			...
			// 바이트 기반 InputStreamReader을 연결 할 수 있도록
			// InputStreamReader를 함께 사용한다
			...
			//PrintWriter.peintln(문자열) 
			//: 주어진 데이터를 문자형태로 출력하고, 출력 후 줄바꿈 추가함
			pw.println(str);// 서버-> 클라이언트로 대이터를 출력(전송)
			pw.flush(); // flush(): 스트림 내용을 밀어냄
			
			//-------------------------------------------------
			
			//클라이언트 -> 서버 메시지 전달 받기(읽기 : 입력)
			String clientMassge = br.readLine();
			
			//서버의 콘솔창에 클라이언트가 보낸 메시지와 IP 전달
			String clientIP = clientSocket.getInetAddress().getHostAddress();
			//getInetAddress() 
			//-> 클라이언트에 대한 IP 주소 정보를 포함한 InetAddress객체 반환
			//getInetAddress()
			//-> InetAddress 객체에서 IP 주소를 문자열 형식으로 반환
			
			System.out.println(clientIP+"가 보낸 메시지 : "+clientMassge);
